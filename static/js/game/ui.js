export function createUI() {
  ...
  let bossBarEl = null;
  let lastZombiesEl = null;
  let waveStatsEl = null;
  let waveStatsTimer = null;
  ...
  function setBossBar(boss) {
    ...
    const phaseLabel = boss.phase > 0 ? ` · 阶段${boss.phase + 1}` : '';
    document.getElementById('boss-bar-text').textContent = `${Math.round(boss.hp)} / ${boss.maxHp}${phaseLabel}`;
  }

  function ensureLastZombies() {
  ...
  function showWaveStats(wave, stats, myId) {
    ensureWaveStats();
