...
assert.strictEqual(
  interpolation.pushSample(entity, 100, 0, { t: 100, snapDistance: 120, maxSamples: 8 }),
  'sample'
);
...
assert.strictEqual(
  interpolation.pushSample(entity, 500, 0, { t: 200, snapDistance: 120, maxSamples: 8 }),
  'snap'
);
...
assert.deepStrictEqual(capped.samples.map((s) => s.x), [7, 8, 9, 10]);
